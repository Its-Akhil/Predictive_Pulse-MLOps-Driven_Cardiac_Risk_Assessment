...
default_run_id = os.environ.get("MLFLOW_RUN_ID", "PASTE_YOUR_BEST_RUN_ID_HERE")
run_id_input = st.sidebar.text_input("Enter MLflow Run ID", default_run_id)


@st.cache_resource
def load_model_from_mlflow(run_id):
    '''
    Loads an MLflow model using the run_id provided.
    Returns model object on success, or None on failure (and logs error into Streamlit sidebar).
    '''
    if not run_id or run_id == "PASTE_YOUR_BEST_RUN_ID_HERE":
        st.sidebar.warning("Please enter a valid MLflow Run ID.")
        return None

    try:
        # Set the tracking URI—should be pointed to your (or user's) MLflow server
        mlflow.set_tracking_uri("http://localhost:5000")  # Change if using remote MLflow server

        # This URI works both locally and remotely
        model_uri = f"runs:/{run_id}/model"
        st.sidebar.info(f"Trying to load model from: {model_uri}")
        model = mlflow.pyfunc.load_model(model_uri)
        st.sidebar.success(f"✅ Model loaded from run ID: {run_id}")
        return model
...
model = load_model_from_mlflow(run_id_input)

if model:
    st.sidebar.success(f"Model loaded successfully from Run ID: `{run_id_input}`")
else:
